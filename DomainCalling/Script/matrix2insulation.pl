...
@fullScriptPathArr=@fullScriptPathArr[0..@fullScriptPathArr-2];
...
if($numContigs == 1) {
    ...
    print("Rscript '".$scriptPath."/R/matrix2insulation.R' '".$cwd."' '".$insulationFile."' ".$headerSizing." ".$headerSpacing." ".$insulationSquare." ".$insulationSquare_bpSize." ".$imageWidth." ".$yBound." ".$transparentBGFlag." > /dev/null");
    print STDERR "\tdone\n" if($verbose);
}
