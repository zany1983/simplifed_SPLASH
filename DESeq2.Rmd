...
sample_info <- data.table::fread("../../../LibInfo.txt",sep = "\t")
...
read_dds_compareFC_baseMean <- function(dds="dds 1 1 .rds"){
  library(DESeq2)
  dds <- readRDS(dds)
...
#                          LvsC = ifelse(deseq_LvsC@listData$padj < 0.05,deseq_LvsC@listData$log2FoldChange,0),
#                          VvsC = ifelse(deseq_VvsC@listData$padj < 0.05,deseq_VvsC@listData$log2FoldChange,0),
#                          VvsL = ifelse(deseq_VvsL@listData$padj < 0.05,deseq_VvsL@listData$log2FoldChange,0)) %>% reshape2::melt(id.vars="contact",variable.name="compare",value.name="log2FC") %>% 
...
data.table::fwrite(d2,file=paste0(x,"_",".cdt"),sep = "\t",quote = F)
 ...
 x <- levels(factor(baseMean_FC_compare$compare))[[1]]
```
