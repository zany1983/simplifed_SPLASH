...
```{r}
export_cdt(bin1 = c(12000,15000),bin2 = c(12000,15000))
```

```{r}
export_cdt(bin2 = c(0,100),bin1 = c(21400,22000))
...
```{r}
library(readxl)
COVID_Normalize <- read_excel("~/Documents/scientific/cronovirus/G3D/nCov-hiR/ctd_files/COVID Normalize.xlsx", 
    sheet = "collaspe")
...
```{r}
export_gdata_rpm <- function(data= merged.matrix,bin1cut=c(0,100),bin2cut=c(0,100),value.var="n",multiple=1e6){
  levels <- 
    cdt<-  lapply(names(data),FUN = function(x){
      
      data <- data[[x]] %>% as.data.frame() %>% mutate(bin2=Arm5,bin1=Arm3) # 3'-5' chimeras
      data.table <- data.table::data.table(bin1=factor(data$bin1,levels = (as.integer(bin1cut[1]/10):as.integer(bin1cut[2]/10))*10),
                                           bin2=factor(data$bin2,levels = (as.integer(bin2cut[2]/10):as.integer(bin2cut[1]/10))*10),
                                           value.var= multiple*data[[value.var]]) %>% na.omit() %>% mutate(sample=x)
 }) %>% bind_rows()
}
...
CDS.anno <- data.table::fread("/Users/yan/Documents/scientific/cronovirus/genome_analysis/SARS2.Narry.Annotation.txt",sep = "\t",header = T)
